...
$Def \; \Rho(j,i)$ 是节点j到i之间的有向路径，j到i的这条通路上有$(v_1,v_2), ...,(v_{n-1},v_n) where \; v_1=j, v_n=i, n \ge 2$
$$
Def: Directed\; Paths上Jacobians的积
\\ \prod\limits_{(a,b)\in \Rho} = J^{a\rarr b}(\alpha^b)=J^{v_{n-1}\rarr v_n}(\alpha^{v_n})\times J^{v_{n-2}\rarr v_{n-1}}(\alpha^{v_{n-1}})...\times J^{v_{1}\rarr v_2}(\alpha^{v_2})
$$
